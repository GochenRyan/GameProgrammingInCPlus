...
bool Game::Initialize() {
	...
	for (int i = 0; i < 7; ++i) {
		Ball ball;
		Vector2 ballPos, ballVel;
		ballPos.x = 1024.0f / 2.0f + i * 20;
		ballPos.y = 768.0f / 2.0f;
		ball.mBallPos = ballPos;
		ballVel.x = -200.0f + i * 20;
		ballVel.y = 235.0f;
		ball.mBallVel = ballVel;
...
void Game::UpdateGame() {
	...
	for (int i = 0; i < mBalls.size(); ++i) {
		...
		// Bounce if needed
		float leftDiff = mLeftPaddlePos.y - mBalls[i].mBallPos.y;
		leftDiff = (leftDiff > 0.0f) ? leftDiff : -leftDiff;
		if (leftDiff <= paddleH / 2.0f && mBalls[i].mBallPos.x <= 25.0f && mBalls[i].mBallPos.x >= 20.0f && mBalls[i].mBallVel.x < 0.0f) {
			mBalls[i].mBallVel.x *= -1.0f;
		}
		else if (mBalls[i].mBallPos.x < 0.0f) {
			mIsRunning = false;
		}

		float RightDiff = mRightPaddlePos.y - mBalls[i].mBallPos.y;
		RightDiff = (RightDiff > 0.0f) ? RightDiff : -RightDiff;
		if (RightDiff <= paddleH / 2.0f && mBalls[i].mBallPos.x >= 999.0f && mBalls[i].mBallPos.x <= 1004.0f && mBalls[i].mBallVel.x > 0.0f) {
			mBalls[i].mBallVel.x *= -1.0f;
		}
		else if (mBalls[i].mBallPos.x > 1024.0f) {
			mIsRunning = false;
		}


		if (mBalls[i].mBallPos.y <= thickness && mBalls[i].mBallVel.y < 0.0f) {
			mBalls[i].mBallVel.y *= -1.0f;
		}
		else if (mBalls[i].mBallPos.y >= (768 - thickness) && mBalls[i].mBallVel.y > 0.0f) {
			mBalls[i].mBallVel.y *= -1.0f;
		}
	}
...
void Game::GenerateOutput() {
	...
	for (int i = 0; i < mBalls.size(); ++i) {
		SDL_Rect ball{
		static_cast<int>(mBalls[i].mBallPos.x - thickness / 2),
		static_cast<int>(mBalls[i].mBallPos.y - thickness / 2),
		thickness,
		thickness
		};
